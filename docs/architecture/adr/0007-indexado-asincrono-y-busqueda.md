...
- (−) **No es un *transactional outbox* completo:** `DispatchAfterCurrentBus` cubre el orden commit→publish, pero si el proceso muere **entre** el commit y el envío al broker, ese evento se pierde. Se acepta para el alcance Must; el reindexado completo (`search:reindex`, HU-L3-E1-02, *Should*) es la red de seguridad para reconstruir el índice.
...
- **Transactional outbox** (tabla de eventos + relay que publica tras commit): elimina por completo la ventana de pérdida del punto (−). Pospuesta: aporta una tabla, un relay y operación extra por una garantía que, para esta prueba, cubre el reindexado completo.
